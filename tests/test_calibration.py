...
class CalibrationDataTests(unittest.TestCase):
    ...
    def test_lookup_per_ifo_version_does_not_cross_contaminate(self, mock_glob):
        """
        Requesting different calibration versions for different
        interferometers in a single call must not let one interferometer's
        version leak into, or overwrite, another's.
        """
...
class FindCalibrationsOnCitTests(unittest.TestCase):
    ...
    def test_o1_does_not_raise(self):
        ...
        the O1 branch was a standalone ``if`` (not chained into the
        if/elif ladder that initialises ``data``) and never set ``data``
        itself.
        """
    ...
    def test_o4_passes_interferometers_and_version_through(self, mock_o4):
        ...
        mock_o4.assert_called_once_with(
            os.path.join(os.path.sep, "home", "cal", "public_html", "archive"),
            1400000000,
            {"H1": "v2", "L1": "v1"},
            interferometers=["H1"],
        )
