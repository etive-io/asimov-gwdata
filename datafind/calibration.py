...
def get_o4_style_calibration(dir, time, version="v1", interferometers=None):
    ...
    version : str or dict
       ...
       (e.g. ``{"H1": "v2", "L1": "v1"}``), allowing different interferometers
       to use different calibration versions within a single call.
    interferometers : list, optional
       ...
       retrieved separately from frame files). Defaults to the keys of
...
def find_calibrations_on_cit(time,
    ...
    interferometers : list, optional
       ...
       to the keys of ``version`` if it is a dict, otherwise to ``["H1", "L1"]``
       (Virgo's O2/O3 envelope is only included if ``V1`` is explicitly listed
       here, since O4-era Virgo calibration is retrieved separately from
       frame files via :func:`get_calibration_from_frame`).
    """
    if interferometers is None:
        if isinstance(version, dict):
            interferometers = list(version.keys())
